...
- **Carousing takes time.** Resolving a session advances the world clock by the selected tier's event bonus in days, with a minimum of one day, in both Original and Expanded modes. With Shadowdark Enhancer's off-duty clock API, carried torches go out without losing their remaining burn time; a refusal never falls back to a plain clock move. Without that API, Foundry advances time normally. ([#151](https://github.com/DimitroffVodka/shadowdark-extras/issues/151))
