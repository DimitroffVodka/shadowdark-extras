...
- **Carousing session edits never actually deleted anything, stranding results and resurrecting cleared state.** Foundry merges flag objects on write, so removing a key requires the `-=` prefix — but five carousing functions mutated a local copy of the session and wrote it back, meaning every `delete` only ever changed the in-memory object. It looked correct until the next reload, when the "removed" data came back. Concretely: unconfirming a player did not persist; clearing a modifier left the old value; clearing or changing a character drop left a stale confirmation and result behind; `pruneOfflineCarousingData` pruned nothing; and removing a GM-managed participant dropped the actor from the list (arrays do replace wholesale) while orphaning its result — a result still in the session but with no participant to render it, so it could never be reviewed or applied. All five now use the `-=` deletion form. Verified against the document source rather than the in-memory copy, since `getFlag` returns a live reference that makes a failed delete look like it worked.
