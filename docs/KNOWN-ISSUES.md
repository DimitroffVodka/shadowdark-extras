...
| 13 | Unidentified magical item sheets exposed `system.magicItem` to non-GM players because SDX wrapped the generic ItemSheet before Shadowdark's ItemSheetSD added the `system` context; the SDX ItemSheetSD subclass now masks only a cloned view context for non-GM unidentified items while preserving GM/identified behavior and the Item document | issue #62, Phase 5.3 |
