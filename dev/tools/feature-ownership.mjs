...
export const FEATURE_OWNERS = {
  ...
  npc: [
    ...
    /**
     * PHASE 0 FINDING — provisional owner, confirm before Phase 2.
     *
     * `data/creature-type-map.mjs` lives in the REPO-ROOT `data/` directory,
     * not `scripts/data/`, and is absent from both the feature map's inventory
     * and the plan's target layout. It is shipped runtime ESM with exactly one
     * consumer, `CreatureTypesApp` (npc), so npc is the only owner consistent
     * with the map's own "graduates to shared at its second consumer" rule.
     *
     * Recorded here so the matrix stops reporting an unclassified target; the
     * feature map carries the same note. The plan's rule stands: a move needs
     * the map updated first, and this is the map being updated.
     */
