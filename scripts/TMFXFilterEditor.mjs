...
export function filterEditor(placeable, sourceBounds) {
    placeable = placeable ?? getTokenMagic().getControlledPlaceables()[0];
    if (!placeable) return;
...
export class FilterSelector extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    static async _onDelete(event) {
        ...
        await getTokenMagic().deleteFilters(this._document, filterId, filterType, filterInternalId);
    ...
    static async _onToggle(event) {
        const { filterId, filterType, filterInternalId } = event.target.closest('.filter').dataset;
        await getTokenMagic().updateFiltersByPlaceable(this._document, [
            {
                filterId,
                filterType,
                filterInternalId,
    ...
    static _onRandomize() {
        ...
        if (paramArray?.length) getTokenMagic().updateFiltersByPlaceable(this._document, paramArray);
    }
...
export class FilterEditor extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    _readParams() {
        ...
        const defaults = getFilterType()[this._filterIdentifier.filterType]?.defaults;
        if (defaults) {
...
export class AnimationEditor extends HandlebarsApplicationMixin(ApplicationV2) {
...
export class RandomizationEditor extends HandlebarsApplicationMixin(ApplicationV2) {
    static genId(document, { filterId, filterType, filterInternalId, param }) {
...
export class SavePreset extends HandlebarsApplicationMixin(ApplicationV2) {
    constructor(document, displayMacro = false) {
        ...
        if (getFilterType()[this._name]) this._name = 'NewFilter';
    ...
    static async _onSave(event) {
        ...
        const preset = getTokenMagic().getPreset({ name, library });
        if (preset) {
            ...
            if (!overwrite) return;
            await getTokenMagic().deletePreset({ name, library }, true);
        }

        await getTokenMagic().addPreset({ name, library, defaultTexture }, params);
        this.close(true);
    }
}
