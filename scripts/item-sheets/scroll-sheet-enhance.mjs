...
/**
 * Scroll item sheet enhancement.
 *
 * Extracted from the composition root in Phase 3, verbatim. Builds the Activity
 * tab — damage/heal, conditions, summoning, item-give, targeting, template and
 * Animation FX — then hands the rendered markup to the shared Activity-tab
 * widget wiring.
 *
 * Scrolls take the jQuery `ItemSheetSD` path, so unlike Potions this one does
 * fire through the root's `renderItemSheet` dispatch in a stock world.
 *
 ...
 */
export async function enhanceScrollSheet(app, html) {
