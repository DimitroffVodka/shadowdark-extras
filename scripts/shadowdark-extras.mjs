 ...
 */
function showConditionsModal(actor, conditionDataMap, theme) {
 ...
 */
function injectSkillsBox(html, actor) {
	if (actor.type !== "Player") return;
 ...
 */
function enhanceGemSheet(app, html) {
 ...
 */
function enhanceGemBag(app, html) {
 ...
 */
function enhanceGemInventory(app, html, actor) {
	if (actor.type !== "Player") return;
 ...
 */
function injectSpellAlignmentField(app, html) {
 ...
 */
function injectStaffSpellButton(app, html, item) {
	// Only for Weapon type items
	if (item.type !== "Weapon") return;
 ...
 */
async function injectStaffSpellsUI(sheet, html, data) {
 ...
 */
function injectWeaponSpellRechargeButtons(app, html, actor) {
...
//
// `checkEffectRequirements` is handed in rather than imported: the equip hook
// needs it, but it belongs to the source-requirement group still in this file,
// with seven other call sites here. Importing it back would be a root-and-back
// cycle; splitting the handler would turn three sequential awaits into two
// concurrent registrations. Replace the argument with an import when that group
// moves to effects/.
registerEffectTriggerHooks({ checkEffectRequirements });
