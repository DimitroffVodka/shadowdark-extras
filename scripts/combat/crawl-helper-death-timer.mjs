 ...
 * else.
 *
 * VERIFICATION DEBT, STATED PLAINLY. `shadowdark-crawl-helper` is installed
 * but DISABLED in the verification world, so only the guard path has been
 * exercised since the move. This is the second unit in that position —
 * `canvas/carousel-drag.mjs` is the first — and both are Phase 4
 * release-readiness rows. A byte-identical carry proves the code did not
 * change; it does not prove it works.
 *
