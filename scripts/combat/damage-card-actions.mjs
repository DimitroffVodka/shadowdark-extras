...
async function trackSummonedTokensForExpiry(sceneId, tokenIds, expiry, spellName) {
	...
	existingList.push({ tokenIds, ...basis, spellName });
	...
	try {
		...
		if (creatures && creatures.length > 0) {
			...
			if (hasDuration) {
				...
				// Auto-delete tokens when the duration runs out.
				// summoningConfig.deleteAtExpiry defaults to true for summoning spells.
				//
				// This used to be gated on `game.combat`, so a summon conjured outside
				// an encounter was never tracked and simply stayed forever. The basis
				// now falls back to world time, which means the duration is honoured
